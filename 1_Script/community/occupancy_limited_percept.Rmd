...
species_cols  <- names(occupancy_data)[1:59]
# print(species_cols)
...
# Verify structure
dim(y_array) # Num of species * num of sites * num of replicates (visits) # 04/18: 59*85*1

# Prepare `occ.covs`: covariate for occurrence
occ_covs <- as.matrix(covariates$Year_since_logging)
dim(occ_covs) # 85 sites
...
det_covs <- list(doy = as.matrix(covariates$doy))
colnames(det_covs$doy) <- "V1" # visit 1
View(det_covs$doy)
str(det_covs)
...
occ.ms.formula <- ~ scale(Year_since_logging) + scale(I(Year_since_logging^2))
...
library(coda)
library(boot)
densplot(model_ms$beta.samples)
...
# species_names <- species_cols[species_cols != "SM2"]
...
length(model_ms$psi.samples) #10030000
...
for (species in 1:num_species) {
    ...
    for (site in 1:85) {
        ...
        species_results[site, "Q0.25"] <- quantile(site_data, probs = 0.025)
...
print(species_dfs[[41]])
ggplot(species_dfs[[21]], aes(x = Patch, y = Mean)) +
    geom_line() +
    labs(title = "Mean Psi Across year since logging for ALFL", x = "Area", y = "Mean Psi")
...
p <- ggplot(combined_df, aes(x = Area, y = Median)) +
  geom_line(color = "blue") +
  geom_ribbon(aes(ymin = Q0.25, ymax = Q97.5), fill = "blue", alpha = 0.2) +  # Adding ribbon for CI
  # geom_point(color = "red") +
  facet_wrap(~ Species, scales = "free_y") +  # 'free_y' allows different y scales for each plot
  labs(x = "Area", y = "Mean Psi") +
  theme_minimal() +
  theme(strip.text.x = element_text(size = 8),  # Adjust font size for facet labels as needed
