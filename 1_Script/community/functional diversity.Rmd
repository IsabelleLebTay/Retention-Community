...
predict_data_edge$lwr <- predicted_no_re$fit - 1.96*predicted_no_re$se.fit
predict_data_edge$upr <- predicted_no_re$fit + 1.96*predicted_no_re$se.fit
...
# Same as top model. nothing significant
Func_diversity_glm9 <- glmmTMB(FRic ~ Year_since_logging*Patch,
                        data = FuncDiv_Covs,
                        family = gaussian)
AIC(Func_diversity_glm9) # -1739.802
...
Func_diversity_glm8 <- glmmTMB(FRic ~ Edge + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2),
                        data = FuncDiv_Covs,
                        family = gaussian)
AIC(Func_diversity_glm8) # -1739.5
summary(Func_diversity_glm8)
```
...
Func_diversity_glm0b <- glmmTMB(FRic ~ Edge,
                        data = FuncDiv_Covs,
                        family = gaussian)
AIC(Func_diversity_glm0b) # -1741.9
summary(Func_diversity_glm0b)
...
Func_diversity_glm6a <- glmmTMB(FRic ~ Edge + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2),
                        data = FuncDiv_Covs,
                        family = beta_family(link="logit"))
AIC(Func_diversity_glm6a) # -1621.556
summary(Func_diversity_glm6a)
r.squaredGLMM(Func_diversity_glm6a) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm6a)) # Residuals are all over the place.
...
summary(Func_diversity_glm6a)
r.squaredGLMM(Func_diversity_glm6a) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm6a)) # Residuals are okFunc_diversity_glm6b

Func_diversity_glm6b <- glmmTMB(FRic ~ RETN_m2 + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2),
                        data = FuncDiv_Covs,
                        family = gaussian)
AIC(Func_diversity_glm6a) # -1621.556
summary(Func_diversity_glm6b)
r.squaredGLMM(Func_diversity_glm6b) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm6b)) # fine
```
...
Edge_plot_scaled <- predict_data_edge %>%
  ...
  theme(legend.position = "none", 
        ...
        plot.background = element_rect(fill = 'white', colour = 'white'),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        plot.margin = unit(c(0.4,1.1,0.4,0.4), "cm")))
...
# Plot from the full model
Func_diversity_glm13a <- glmmTMB(FRic ~ RETN_m2 + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2),
                        data = FuncDiv_Covs,
                        family = gaussian)

func_rich_summary <- summary(Func_diversity_glm13a)

# Extract the coefficients table (fixed effects as an example)
coefficients_table <- as.data.frame(func_rich_summary$coefficients$cond) # for conditional effects

# Save the coefficients table to a CSV file
write.csv(coefficients_table, file.path("2_Outputs/Functional", "functional_richnness_fullModel_summary.csv"), row.names=TRUE)

# Create a new data frame for predictions
predict_data_area <- data.frame(
    RETN_m2 = seq(min(FuncDiv_Covs$RETN_m2, na.rm = TRUE), 
              max(FuncDiv_Covs$RETN_m2, na.rm = TRUE), length.out = 367),
    Patch = mean(FuncDiv_Covs$Patch, na.rm = TRUE),
    Year_since_logging = mean(FuncDiv_Covs$Year_since_logging, na.rm = TRUE),
    Dist_near_forest = mean(FuncDiv_Covs$Dist_near_forest, na.rm = TRUE),
    Year_since_logging2 = mean(FuncDiv_Covs$Year_since_logging2, na.rm = TRUE))

# Predict using the model without a random effect
predicted_no_re <- predict(Func_diversity_glm13a, newdata = predict_data_area, type = "response", se.fit = TRUE)

# Add predictions and confidence intervals to the data frame
predict_data_area$lwr <- predicted_no_re$fit - 1.96*predicted_no_re$se.fit
predict_data_area$upr <- predicted_no_re$fit + 1.96*predicted_no_re$se.fit
predict_data_area$predicted <- predicted_no_re$fit
# View(predict_data_area)

# Plotting harvest age
# Scaled back
predict_data_area$Original_Area <- predict_data_area$RETN_m2 * 12000

# Update plotting code
Area_plot_scaled <- predict_data_area %>%
  ggplot(aes(x = Original_Area, y = predicted, group = 1)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = 'grey', alpha = .4, linetype = 0) +
  geom_line() +
  geom_point(data = FuncDiv_Covs, 
            aes(x = RETN_m2 * 12000, y = FRic, group = 1), 
            size = 0.5,
            alpha = .3,
            position = "jitter") + # Adjust for actual data points if necessary
  theme_minimal() +
...
ggsave(filename = file.path("2_Outputs/Functional", "Functional richness and area.png"), plot = Area_plot_scaled, width = 5, height = 4)
...
# patch size is negative on evenness
Func_diversity_glm13 <- glmmTMB(FEve ~ RETN_m2 + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2),
                        data = FuncDiv_Covs,
                        family = gaussian)

summary(Func_diversity_glm13)
r.squaredGLMM(Func_diversity_glm13) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm13)) # not great

Func_diversity_glm13a <- glmmTMB(FEve ~ RETN_m2 + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2) + (1|location),
                        data = FuncDiv_Covs,
                        family = Gamma(link="logit"))

summary(Func_diversity_glm13a)
r.squaredGLMM(Func_diversity_glm13) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm13a))# finally they are good

Func_diversity_glm13b <- glmmTMB(FEve ~ Edge + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2) + (1|location),
                        data = FuncDiv_Covs,
                        family = Gamma(link="logit"))

summary(Func_diversity_glm13b)
r.squaredGLMM(Func_diversity_glm13) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm13b))# finally they are good

```

## Functional evenness: comparing top models
```R 
# New best
head(FuncDiv_Covs)
Func_diversity_glm8b <- glmmTMB(FEve ~ Edge + Year_since_logging*Patch + Veg_cat*Patch,
...
## FEve: plotting full model 
```R 
Func_diversity_glm13a <- glmmTMB(FEve ~ RETN_m2 + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2) + (1|location),
                        data = FuncDiv_Covs,
                        family = Gamma(link="logit"))

summary(Func_diversity_glm13anore)
r.squaredGLMM(Func_diversity_glm13) # 0.01930648 0.01930648
plot(simulateResiduals(Func_diversity_glm13a))# finally they are good
Func_diversity_glm13anore <- glmmTMB(FEve ~ RETN_m2 + Year_since_logging*Patch + Dist_near_forest + poly(Year_since_logging2),
                        data = FuncDiv_Covs,
                        family = Gamma(link="logit"))
Func_diversity_glm0b <- glmmTMB(FRic ~ Edge,
                        data = FuncDiv_Covs,
...
r.squaredGLMM(Func_diversity_glm0b)
  # Save the top model output
func_eve_summary <- summary(Func_diversity_glm13a)

# Extract the coefficients table (fixed effects as an example)
coefficients_table <- as.data.frame(func_eve_summary$coefficients$cond) # for conditional effects

# Save the coefficients table to a CSV file
write.csv(coefficients_table, file.path("2_Outputs/Functional", "functional_evenness_summary.csv"), row.names=TRUE)
...
# Predict using the model without a random effect
predicted_no_re <- predict(Func_diversity_glm13anore, newdata = predict_data_area, type = "response", se.fit = TRUE)
...
Area_plot_scaled <- predict_data_area %>%
  ...
  geom_line() +
  geom_point(data = FuncDiv_Covs, 
            aes(x = RETN_m2 * 12000, y = FEve, group = 1), 
            size = 0.5,
            alpha = .3,
            position = "jitter") + # Adjust for actual data points if necessary
  theme_minimal() +
...
print(Area_plot_scaled)
ggsave(filename = file.path("2_Outputs/Functional", "Functional evenness and area.png"), plot = Area_plot_scaled, width = 5, height = 4)
...
for (bird in species_list) {
  ...
  dredge_edge <- dredge(global.model = edge_gm, rank = 'AICc', extra = list(
                          "adjR^2",
                          "mcfaddR^2" = function(x) {PseudoR2(x, which = "McFaddenAdj")},
                          "deltaR2m" = function(x) {r.squaredGLMM(x)[1]}
                          ))
  ...
  write.csv(edge_top, glue("2_Outputs/Single species/Dredge tables/R2 values/{bird}_edge_R2_nbinom.csv"))
  write.csv(area_top, glue("2_Outputs/Single species/Dredge tables/R2 values/{bird}_area_R2_nbinom.csv"))
}
```
...
```R 
ALFL <- glm(ALFL ~ Dist_near_forest + poly(Year_since_logging2) + Year_since_logging*Patch,
                data = Abund_Covs,
                family = poisson)
AIC(ALFL)
summary(ALFL)
...
CHSP <- glmmTMB(CHSP ~ Dist_near_forest + poly(Year_since_logging2) +Year_since_logging + Veg_cat,
                data = Abund_Covs,
                family = poisson)
summary(CHSP)
...
COYE <- glmmTMB(COYE ~ poly(Year_since_logging2) + Veg_cat +Year_since_logging*Patch,
                data = Abund_Covs,
                family = poisson)
summary(COYE)
...
LEFL <- glmmTMB(LEFL ~ Dist_near_forest + RETN_m2 +Veg_cat +Year_since_logging + Patch,
                data = Abund_Covs,
                family = poisson)
summary(LEFL)
...
LISP <- glmmTMB(LISP ~ poly(Year_since_logging2) + Year_since_logging + RETN_m2,
                data = Abund_Covs,
                family = poisson)
summary(LISP)
...
MOWA <- glmmTMB(MOWA ~ poly(Year_since_logging2) + Veg_cat + Patch,
                data = Abund_Covs,
                family = poisson)
summary(MOWA)
...
REVI <- glmmTMB(REVI ~ Dist_near_forest + Edge + poly(Year_since_logging2) + Veg_cat + Year_since_logging,
                data = Abund_Covs,
                family = poisson)
summary(REVI)
...
# Residuals not behaving as expected as Poisson. Nge Binom fixes it!
TEWA <- glmmTMB(TEWA ~ Dist_near_forest + poly(Year_since_logging2) + Veg_cat + Year_since_logging*Patch + (1|location),
                data = Abund_Covs,
                family = poisson)
TEWA <- glmmTMB(TEWA ~ Dist_near_forest + poly(Year_since_logging2) + Veg_cat + Year_since_logging*Patch,
                data = Abund_Covs,
                family = nbinom1)
summary(TEWA)
r.squaredGLMM(TEWA)
...
write.csv(coefficients_table, file.path("2_Outputs/Single species/Model summaries", "TEWA_summary_nbinom.csv"), row.names=TRUE)
...
WAVI <- glmmTMB(WAVI ~ Dist_near_forest + RETN_m2,
                data = Abund_Covs,
                family = poisson)
YEWAnoveg <- glmmTMB(YEWA ~ RETN_m2 + Patch,
                data = Abund_Covs,
                family = poisson)
# Create a new data frame for predictions
predict_data_edge <- data.frame(
...
# Predict using the model without a random effect and without categotircal 
predicted_fit <- predict(YEWAnoveg, newdata = predict_data_edge, type = "response", se.fit = TRUE)
...
ggsave(filename = file.path("2_Outputs/Single species/Area plots", "YEWA and area.png"), plot = area_plot_scaled, width = 5, height = 4)

```

## Highlighted species: harvest year * Patch plots
```R 
COYE <- glmmTMB(COYE ~ Dist_near_forest + poly(Year_since_logging2) + Veg_cat + Year_since_logging*Patch,
                data = Abund_Covs,
                family = poisson)

p <- plot_model(COYE, type = "int", 
           title = "",          # Removes the plot title
           axis.title = c("Year since harvest", "Common Yellowthroat abundance"),
            show.legend = FALSE)
p <- p + scale_x_continuous(breaks = seq(0, 1, length.out = 5),
                       labels = seq(0, 22, length.out = 5)) + 
          scale_color_manual(values = c("0" = "grey88", 
                                  "1" = "grey37")) +              
      theme(legend.position = "right", # Adjust the legend position: 'left', 'right', 'top', 'bottom', or c(x, y)
        legend.title = element_text(color = "black", size = 12, face = "bold"), # Customize legend title
        legend.text = element_text(color = "black", size = 10), # Customize legend text
        legend.background = element_rect(fill = "white", size = 0.5, linetype = "solid"), # Legend background
        legend.key = element_blank(),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank()
                          ) + # Remove background of the legend keys
  guides(fill = guide_legend(title = "Patch", # Customize legend title
                             override.aes = list(size = 3))) # Customize legend symbols size
print(p)


ggsave(filename = file.path("2_Outputs/Single species/Year x Patch plots", "COYE year x patch.png"), plot = p, width = 5, height = 4)
...
## Plot covariate allocation 
```R 
prop_cv <- read.csv("2_Outputs/Single species/covariate proportion.csv")
# Plotting
labels <- prop_cv$covariate
labels[labels == "Year^2"] <- "Year²"
labels[labels == "Patch area (m^2)"] <- "Patch area (m²)"
print(labels)
cov_barplot <- ggplot(prop_cv, aes(x=covariate, y=percent)) + 
  geom_bar(stat="identity", fill="grey") +
  scale_x_discrete(limits=prop_cv$covariate, labels=labels) + # Customizing x-axis labels
  theme_minimal() +
  labs(x="Covariate", y="Proportion") +
...
ggsave(filename = file.path("2_Outputs/Single species", "Covariate allocation barplot.png"), plot = cov_barplot, width = 5, height = 4)
