...
# Did it with the birds_abund_covs_noLargePatches df by mistake, but it shouldn't matter too much. Will have to switch for evenness
...
# Richness: Top model and plots 

```R 
# This is the best model
hill_rich5re <- glmmTMB(richness ~ RETN_m2 + Year_since_logging + Dist_near_forest + poly(Year_since_logging2) + (1|location),
                        data = birds_abund_covs_noLargePatches,
                        family = poisson)

# Save the top model output
hill_rich5re_summary <- summary(hill_rich5re)
# Extract the coefficients table (fixed effects as an example)
coefficients_table <- as.data.frame(hill_rich5re_summary$coefficients$cond) # for conditional effects
# If you want to save random effects or other parts, adjust accordingly

# Save the coefficients table to a CSV file
write.csv(coefficients_table, file.path("2_Outputs/Taxonomic", "hill_rich5re_summary.csv"), row.names=TRUE)

## Retyring to predict data and plot
# exlcude the ranodm effects
hill_rich_no_re <- glmmTMB(richness ~ RETN_m2 + Year_since_logging + Dist_near_forest + poly(Year_since_logging2),
                        data = birds_abund_covs_noLargePatches,
                        family = poisson)
...
predict_data_age2 <- data.frame(
  ...
  RETN_m2 = mean(birds_abund_covs_noLargePatches$RETN_m2, na.rm = TRUE),
...
# Plotting
Age2_plot <- predict_data_age2 %>%
  ggplot(aes(x = Year_since_logging, y = predicted)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = 'grey', alpha = .3, linetype = 0) + 
  geom_line()+
  geom_point(data = birds_abund_covs_noLargePatches, aes(x = sqrt(Year_since_logging2), y = richness, group = 1), alpha = .7) +
  theme_minimal() +
  ylab("Taxonomic richness") +
  xlab(expression("Year since logging")) +
...
Age2_plot_unscaled <- predict_data_age2 %>%
  ggplot(aes(x = Original_Year_since_logging, y = predicted, group = 1)) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = 'grey', alpha = .3, linetype = 0) +
  geom_line() +
  geom_point(data = birds_abund_covs_noLargePatches, aes(x = Year_since_logging * 22, y = richness, group = 1), alpha = .7) + # Adjust for actual data points if necessary
  theme_minimal() +
  ylab("Taxonomic richness") +
...
print(Age2_plot_unscaled)

ggsave(filename = file.path("2_Outputs/Taxonomic", "Unscaled hill_rich_5_nore effect of harvest age on taxonomic rich.png"), plot = Age2_plot_unscaled, width = 10, height = 6)

plot_model(hill_rich5re, "est")
plot_model(distance_forest_3, type = "std")
```

# Camparing top models
```R
# This is the best model
hill_rich5re <- glmmTMB(richness ~ RETN_m2 + Year_since_logging + Dist_near_forest + poly(Year_since_logging2) + (1|location),
                        data = birds_abund_covs_noLargePatches,
                        family = poisson)
summary(hill_rich5re)
nrow(birds_abund_covs_noLargePatches)
AIC(hill_rich5re) # 1896.409
r.squaredGLMM(hill_rich5re) # delta R2m = 0.2573984
plot(simulateResiduals(hill_rich5re)) # Residuals ok
